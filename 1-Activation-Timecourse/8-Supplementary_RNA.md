...
Get the TPM values for all samples and fit linear model.
...
<img src="8-Supplementary_RNA_files/figure-gfm/unnamed-chunk-4-1.png" style="display: block; margin: auto;" />

``` r
ggsave("Figs/Supplementary1/RNA_volcanos_DE.pdf", width = 12, height = 4)
```
...
dat.rna %>% 
  ...
  summarise(logTPM = median(logTPM)) %>% 
  ungroup() %>% 
  pivot_wider(names_from = c(donor, timepoint), values_from = logTPM) %>% 
  na.omit() %>% 
...
<img src="8-Supplementary_RNA_files/figure-gfm/unnamed-chunk-6-1.png" style="display: block; margin: auto;" />
